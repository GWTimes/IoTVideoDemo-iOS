...
//        deviceIdLabel?.text = device.deviceID + "_\(sourceID)"
        ...
        if let player = player as? IVPlayerTalkable {
            let aencs: [String : IVAudioCodecType] = ["amr": .AMR, "aac": .AAC, "g711a": .G711A]
            
            if let ae = cfgkvs["-aenc"]?.lowercased(), let aenc = aencs[ae] {
                player.audioEncoder.audioType = aenc // 默认AAC
            }
...
extension IVDevicePlayerViewController: IVPlayerDelegate {
    ...
    func connection(_ connection: IVConnection, didUpdateSpeed speed: UInt32) {
        DispatchQueue.main.async {[weak self] in
            ...
            logVerbose(self.speedLabel?.text as Any)
        }
    }
...
//        logInfo(device.deviceID + "_\(sourceID)", "willBeginOfFile:", fileTime)
    }
