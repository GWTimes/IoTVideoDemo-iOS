...
// 是否使用测试服
let using_test_web_host = IVConfigMgr.cfgValue(forKey: IVOptionKey.hostWeb.rawValue) == "TEST"
...
//        UIApplication.shared.clearLaunchScreenCache()
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        // 要获取当前所连WiFi名称的额外条件：
        // - iOS13以后需打开定位服务
        // - iOS14以后需打开精确定位服务，或者申请临时精确定位服务
        if #available(iOS 14.0, *),
           locMgr.accuracyAuthorization == .reducedAccuracy,
           locMgr.authorizationStatus.isEqual(oneOf: .authorizedWhenInUse, .authorizedAlways) {
            locMgr.requestTemporaryFullAccuracyAuthorization(withPurposeKey: "FetchSSID")
