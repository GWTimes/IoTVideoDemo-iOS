...
extension IVTencentNetwork {
    public func request(methodType: IVTencentNetworkRequestType,
        ...
        let success = {(task: URLSessionTask, json: Any?) -> () in
            if json == nil {
                response?(nil, NSError(domain: "", code: 997, userInfo: [NSLocalizedDescriptionKey : json as Any]))
                return
            }
            ...
            if let jsonData = try? JSONSerialization.data(withJSONObject: json!, options: []) {
                ...
                response?(nil, NSError(domain: "", code: 999, userInfo: [NSLocalizedDescriptionKey : json as Any]))
            }
        }
